...
CREATE TABLE IF NOT EXISTS `user_share_procedures` (
  ...
  `charges` DECIMAL(10,2) DEFAULT NULL COMMENT 'The actual charge/price for the procedure in Rs',
  `share_type` ENUM('percentage', 'rupees') NOT NULL DEFAULT 'percentage',
  `share_value` DECIMAL(10,2) NOT NULL COMMENT 'Share value (percentage or amount)',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  INDEX `idx_user_id` (`user_id`),
