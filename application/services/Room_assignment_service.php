...
class Room_assignment_service {
    ...
    public function validate_room_availability($room_id, $date, $time, $duration = 30) {
        ...
        // Check room capacity (if needed in future)
        // For now, we assume one appointment per room at a time
        ...
        $this->CI->db->where('appointment_date >=', $datetime);
        $this->CI->db->where('appointment_date <', $end_datetime);
        $this->CI->db->or_group_start();
        $this->CI->db->where('appointment_end_time >', $datetime);
        $this->CI->db->where('appointment_end_time <=', $end_datetime);
        $this->CI->db->group_end();
        $this->CI->db->group_end();
        
        $conflicts = $this->CI->db->count_all_results('appointments');
        
        if ($conflicts > 0) {
            return array('available' => false, 'message' => 'Room has conflicting appointment');
        }
